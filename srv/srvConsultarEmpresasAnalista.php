...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($data['idUsuario'])) { // Cambio aquí para coincidir con el nombre del parámetro enviado
        $idUsuario = $data['idUsuario']; // Cambio aquí para coincidir con el nombre del parámetro enviado
        //$usuario->cue = $_SESSION[CUE];
        /* $usuario = new Usuario();
        $usuario->cue = $_SESSION[CUE];
        $usuario->cue = $cue; */
        $response=  consultarEmpresaAnalista($idUsuario); // Modificación aquí para enviar el ID como parámetro
